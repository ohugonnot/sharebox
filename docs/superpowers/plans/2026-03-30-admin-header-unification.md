...
**Architecture:** Ajouter `style.css` dans le `<head>` d'`admin.php`, déplacer le `<header class="app-header">` comme premier enfant du `<div class="main">` (il prend ainsi la même contrainte de largeur/padding que le contenu), et supprimer le `<nav>` et son CSS inline. Pas de nouveau wrapper div — `.main` sert déjà de conteneur.
...
Dans le `<style>` inline d'`admin.php`, supprimer entièrement le bloc `/* ── Nav ── */` et toutes ses règles :
...
### Task 3 : Remplacer le `<nav>` par `<header class="app-header">` dans le HTML

**Files:**
- Modify: `admin.php`

- [ ] **Step 1 : Remplacer le bloc HTML**

Remplacer ce bloc (lignes ~640–657) :
```html
<nav class="nav">
    <div class="nav-left">
...
- Logo SVG orange + "ShareBox" en grand + "Administration" en sous-titre gris
- Liens "← Fichiers", ruTorrent (si seedbox), username, Logout à droite
- Même style que `https://dn40904.seedhost.eu/share/`
